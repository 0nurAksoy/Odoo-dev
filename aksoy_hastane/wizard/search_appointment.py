...
class SearchAppointmentWizard(models.TransientModel):
    ...
    def action_search_appointment_m1(self):
        action = self.env.ref('aksoy_hastane.action_hospital_appointment').read()[0]
        action['domain'] = [('patient_id', '=', self.patient_id.id)]
        return action

    def action_search_appointment_m2(self):
        action = self.env['ir.actions.actions']._for_xml_id("aksoy_hastane.action_hospital_appointment")
        action['domain'] = [('patient_id', '=', self.patient_id.id)]
        return action
